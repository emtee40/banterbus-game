...
func TestRoomServiceCreate(t *testing.T) {
	...
	t.Run("Should throw error when fail to create room in DB", func(t *testing.T) {
		...
		ctx := context.Background()
		mockRandom.EXPECT().GetAvatar().Return(newCreatedPlayer.Avatar)
		mockStore.EXPECT().CreateRoom(ctx, newCreatedPlayer, entities.NewRoom{GameName: "fibbing_it"}).Return("", fmt.Errorf("failed to create room"))
...
func TestRoomServiceJoin(t *testing.T) {
	t.Run("Should join room in DB successfully", func(t *testing.T) {
		...
		mockRandom.EXPECT().GetNickname().Return(newCreatedPlayer.Nickname)
		mockRandom.EXPECT().GetAvatar().Return(newCreatedPlayer.Avatar)
		mockStore.EXPECT().AddPlayerToRoom(ctx, newCreatedPlayer, "ABC12").Return([]sqlc.GetAllPlayersInRoomRow{
			{
		...
		assert.NotEmpty(t, room.Players[1].Nickname, "should get a nickname set if the user didn't provide one")
	})
	...
	t.Run("Should throw error when fail to join room in DB", func(t *testing.T) {
		...
		ctx := context.Background()
		mockRandom.EXPECT().GetAvatar().Return(newCreatedPlayer.Avatar)
		mockStore.EXPECT().AddPlayerToRoom(ctx, newCreatedPlayer, "ABC12").Return([]sqlc.GetAllPlayersInRoomRow{}, fmt.Errorf("failed to join room"))
